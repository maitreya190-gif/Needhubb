  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        // "You" tile appears when available
        if (available) _SelfChitChatTile(t: t),

        // Real roster (hydrated by main.dart poller — refreshed every 15s).
        ...chitchatRosterNotifier.value.map(
          (p) => _ChitChatRealTile(person: p, t: t),
        ),

        // Fallback mock list when no real users are visible.
        if (chitchatRosterNotifier.value.isEmpty)
          ..._people.map((p) => _ChitChatPersonTile(person: p, t: t)),
...
class _ChitChatPersonTile extends StatelessWidget {
  final ({
    String initials,
    String name,
    String area,
  ...
  const _ChitChatPersonTile({required this.person, required this.t});
...
class _ChitChatRealTile extends ConsumerWidget {
  final ChitchatPerson person;
  final NeedHubTokens t;

  const _ChitChatRealTile({required this.person, required this.t});
