...
export async function runSeed() {
  ...
  // ── Interest responses (offers on needs) + accepted flows + reviews ──────
  // Pick two open needs and create realistic offer chains that exercise:
  //   response → accept → fulfilled need → DM thread → 5★ review → points award
  const needsForDemo = await prisma.need.findMany({
    where: { posterId: { in: Object.values(usersById) } },
    take: 5,
  })

  if (needsForDemo[0] && meera && rohan) {
    // Offer on Aarav's UI-designer need
    const target = needsForDemo.find((n) => n.title.includes('UI designer'))
    if (target) {
      const resp = await prisma.interestResponse.upsert({
        where: { needId_responderId: { needId: target.id, responderId: meera } },
        update: {},
        create: {
          needId: target.id,
          responderId: meera,
          message: "I'd love to help — I've shipped 3 consumer apps and can start this weekend. Attaching portfolio in DMs.",
          status: 'ACCEPTED',
        },
      })
      await prisma.messageThread.upsert({
        where: { responseId: resp.id },
        create: { responseId: resp.id },
        update: {},
      })
      // Sample messages
      const thread = await prisma.messageThread.findUnique({ where: { responseId: resp.id } })
      if (thread) {
        await prisma.message.deleteMany({ where: { threadId: thread.id } })
        await prisma.message.createMany({
          data: [
            { threadId: thread.id, senderId: meera, body: "Hey! I saw your need — huge portfolio match. Free to hop on a call?" },
            { threadId: thread.id, senderId: target.posterId, body: 'Yes! Tomorrow 6pm works?' },
            { threadId: thread.id, senderId: meera, body: 'Perfect. Sending calendar invite.' },
          ],
        })
      }
      // Mark need fulfilled, add a 5★ review from Aarav to Meera
      await prisma.need.update({ where: { id: target.id }, data: { status: 'FULFILLED' } })
      const existingReview = await prisma.review.findFirst({
        where: { needId: target.id, reviewerId: target.posterId, revieweeId: meera },
      })
      if (!existingReview) {
        await prisma.review.create({
          data: {
            needId: target.id,
            reviewerId: target.posterId,
            revieweeId: meera,
            rating: 5,
            comment: 'Meera was fantastic — thoughtful, fast, and shipped beyond scope. Would 100% work with her again.',
          },
        })
        await prisma.pointsLedger.create({
          data: { userId: meera, delta: 20, reason: 'REVIEW_5STAR', refId: target.id },
        })
        await prisma.profile.update({ where: { userId: meera }, data: { pointsTotal: { increment: 20 } } })
      }
    }
  }

  if (needsForDemo[1] && karthik && priya) {
    // Priya offered on Karthik's guitar-teacher need but not yet accepted (pending)
    const target = needsForDemo.find((n) => n.title.includes('Guitar teacher'))
    if (target) {
      await prisma.interestResponse.upsert({
        where: { needId_responderId: { needId: target.id, responderId: priya } },
        update: {},
        create: {
          needId: target.id,
          responderId: priya,
          message: "I've been teaching guitar for 4 years, mostly acoustic beginners. Free Tue/Thu evenings.",
          status: 'PENDING',
        },
      })
    }
  }
  console.log('  ✓ offers (accepted + pending), DM thread, 1 review, points ledger')

  // ── Achievements (approved) for showcasing user profile richness ─────────
  const achievements = [
    { email: 'aarav.kumar@needhub.demo', title: 'Winner — Flipkart Grid 6.0', category: 'HACKATHON', description: 'First place in the mobile track. Built a fintech app in 48 hours.' },
    { email: 'meera.kulkarni@needhub.demo', title: 'Design Fellowship — Interaction Design Foundation', category: 'AWARD', description: '6-month intensive with a portfolio review by industry design leads.' },
    { email: 'rohan.verma@needhub.demo', title: 'Featured photographer — Nat Geo Your Shot', category: 'AWARD', description: 'Portrait of a Rajasthan potter selected for the monthly editorial.' },
  ]
  for (const a of achievements) {
    const uid = usersById[a.email]
    if (!uid) continue
    const existing = await prisma.achievement.findFirst({ where: { userId: uid, title: a.title } })
    if (!existing) {
  ...
  // ── Sample redemption history so points/redemption feels lived-in ────────
  if (aarav) {
    const item = await prisma.redemptionItem.findFirst({ where: { title: 'Cafe Coffee Day ₹150' } })
    if (item) {
      const existing = await prisma.redemption.findFirst({ where: { userId: aarav, itemId: item.id } })
      if (!existing) {
